...
func (b *BigIP) AddServiceDiscoveryNodes(tenantUri string, config []interface{}) error {
	resp, err := b.postReq(config, uriMgmt, uriShared, "service-discovery", "task", tenantUri, "nodes")
	if err != nil {
		return err
	}
	respRef := make(map[string]interface{})
...
func (b *BigIP) GetServiceDiscoveryNodes(tenantUri string) (interface{}, error) {
	var nodesList interface{}
	err, ok := b.getForEntity(&nodesList, uriMgmt, uriShared, "service-discovery", "task", tenantUri, "nodes")
	if err != nil {
		return nil, err
	}
	if !ok {
